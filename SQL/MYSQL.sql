...
WHERE
    OI.ORDER_ID = (
        SELECT
            OI_SUB.ORDER_ID
        FROM
            ORDER_ITEMS AS OI_SUB
        JOIN
            PRODUCT AS P_SUB
        ON
            OI_SUB.PRODUCT_ID = P_SUB.PRODUCT_ID
        WHERE
            OI_SUB.ORDER_ID = OI.ORDER_ID
        GROUP BY
            OI_SUB.ORDER_ID
        HAVING
            SUM(P_SUB.LEN * P_SUB.WIDTH * P_SUB.HEIGHT) < (
                SELECT
                    LEN * WIDTH * HEIGHT
                FROM
                    CARTON
        ...
        ORDER BY
            SUM(P_SUB.LEN * P_SUB.WIDTH * P_SUB.HEIGHT) DESC
        LIMIT 1
    )
GROUP BY
    OI.ORDER_ID
